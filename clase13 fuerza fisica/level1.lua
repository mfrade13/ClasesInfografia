...
--				timer.performWithDelay( 1000, cambiarCuerpoFisico,1 )
		...
		crate.angularDamping  = 5

		function sceneGroup:touch( event )
			local phase = event.phase
			print("Evento touch para pantalla")
			if phase == "began" then

			elseif phase == "ended" or phase == "cancelled" then
				print(crate.canJump)
				if crate.canJump == true then
					--un impulso en X,Y hara que se impulse hacia la derecha y abajo
					-- por la orientacion de la pantalla
					crate:applyLinearImpulse( 10, 80, crate.x, crate.y )	
					crate.canJump=false --deshabilita el salto del objeto
				end
				-- -- un valor negativo al aplicar fuerza emplea un salto hacia arriba
			--  crate:applyForce( 300, -500, crate.x, crate.y )
